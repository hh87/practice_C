...
void MatMult(complex **a , complex **b ,complex **c , int m, int n ,int p){
	//matrix mult c=a * b,a is m * n matrix , b is n * p matrix,c is m * p matrix
	int Crow,Ccol;
	int Arow,Acol,Brow,Bcol;
	for(Crow = 0 ; Crow < m ;Crow++){
		for(Ccol = 0 ; Ccol < p ; Ccol++){
			 (*((complex*)c + p * Crow + Ccol)).re = 0 ;
			 (*((complex*)c + p * Crow + Ccol)).im = 0 ;
			 for(Acol = 0 ; Acol < n ; Acol++){
				(*((complex*)c + p * Crow + Ccol)) = Cadd((*((complex*)c + p * Crow + Ccol)),Cmult((*((complex*)a + n * Crow + Acol)),(*((complex*)b + p * Acol + Ccol))));
			 }
		}
	}	
}
